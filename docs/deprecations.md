...
## Missing `frameworkVersion`

Starting from v2.0.0, `frameworkVersion` will be required in service configuration, when using globally installed Framework.

It's to ensure that major release upgrades do not accidentally break service deployments (note that new major releases are planned to be published more frequently that it was in a past)
